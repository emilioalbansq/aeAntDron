...
import amDataAccess.Interfaces.IDAO;
...
public class DataHelperSQLiteDAO <T> implements IDAO<T> {
    ...
    public boolean create(T entity) throws AppException {
        ...
        for (Field field : fields) {
            ...
            if (!name.equalsIgnoreCase(tablePK)
                && !name.equalsIgnoreCase("amEstado")
                && !name.equalsIgnoreCase("amFechaCreacion")
                && !name.equalsIgnoreCase("amFechaModifica")) {
    ...
    public boolean update(T entity) throws AppException {
        try {
            ...
            updates.append("amFechaModifica = ?"); // campo técnico de auditoría
    ...
    public boolean delete(Integer id) throws AppException {
        String sql = String.format("UPDATE %s SET amEstado = ?, amFechaModifica = ? amWHERE %s = ?", tableName, tablePK);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            stmt.setString(1, "X");
    ...
    public T readBy(Integer id) throws AppException {
        String sql = String.format("SELECT * FROM %s WHERE %s = ? AND amEstado = 'A'", tableName, tablePK);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            stmt.setInt(1, id);
    ...
    public List<T> readAll() throws AppException {
        ...
        String sql = String.format("SELECT * FROM %s WHERE amEstado = 'A'", tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
    ...
    public Integer getMaxReg() throws AppException {
        String sql = String.format("SELECT COUNT(*) FROM %s WHERE amEstado = 'A'", tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
