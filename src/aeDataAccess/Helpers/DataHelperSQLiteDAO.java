...
import aeDataAccess.Interfaces.IDAO;
import aeInfrastructure.AppConfig;
...
public class DataHelperSQLiteDAO <T> implements IDAO<T> {
    ...
    public boolean create(T entity) throws AppException {
        ...
        for (Field field : fields) {
            ...
            if (!name.equalsIgnoreCase(tablePK)
                && !name.equalsIgnoreCase("aeEstado")
                && !name.equalsIgnoreCase("aeFechaCreacion")
                && !name.equalsIgnoreCase("aeFechaModifica")) {
    ...
    public boolean update(T entity) throws AppException {
        try {
            ...
            updates.append("aeFechaModifica = ?"); // campo técnico de auditoría
    ...
    public boolean delete(Integer id) throws AppException {
        String sql = String.format("UPDATE %s SET aeEstado = ?, aeFechaModifica = ? aeWHERE %s = ?", tableName, tablePK);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            stmt.setString(1, "X");
    ...
    public T readBy(Integer id) throws AppException {
        String sql = String.format("SELECT * FROM %s WHERE %s = ? AND aeEstado = 'A'", tableName, tablePK);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            stmt.setInt(1, id);
    ...
    public List<T> readAll() throws AppException {
        ...
        String sql = String.format("SELECT * FROM %s WHERE aeEstado = 'A'", tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
    ...
    public Integer getMaxReg() throws AppException {
        String sql = String.format("SELECT COUNT(*) FROM %s WHERE aeEstado = 'A'", tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
